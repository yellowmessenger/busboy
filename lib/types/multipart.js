...
function Multipart(boy, cfg) {
  ...
  this.parser.on('drain', function() {
    ...
    part.on('header', function(header) {
      ...
      if (contype === 'application/octet-stream' || filename !== undefined) {
        ...
        onEnd = function() {
          curField = undefined;
          if (buffer.length){
            if (['html', 'text', 'subject', 'to', 'from', 'cc'].includes(fieldname)) {
              fieldsWithCustomCharset[fieldname] = { buffer, truncated, encoding, contype };
            } else {
                buffer = decodeText(buffer, 'binary', charset);
                boy.emit('field', fieldname, buffer, false, truncated, encoding, contype);
                if (fieldname === 'charsets') {
                    try {
                        customCharsetMapping = JSON.parse(buffer);
                    } catch (e) {}
                }
            }
        }
    ...
    Object.keys(fieldsWithCustomCharset).forEach(key => {
        const decodedField = decodeText(fieldsWithCustomCharset[key].buffer, 'binary', customCharsetMapping[key] || defCharset);
        boy.emit('field', key, decodedField, false, fieldsWithCustomCharset[key].truncated, fieldsWithCustomCharset[key].encoding, fieldsWithCustomCharset[key].contype);
      });
    finished = true;
    checkFinished();
  });
